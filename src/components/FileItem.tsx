import { ArrowDownLeft, ArrowUpRight, FileText, Loader } from "lucide-react";
import { useDownloadFile } from "@/hooks/mutations/use-download-file";
...
export function FileItem({ isNew, isOut, it }: FileItemProps) {
	const { mutate: downloadFile, isPending } = useDownloadFile({
		mutationKey: ["download-file", it.id],
		scope: {
			id: it.id,
		},
	});
	...
	return (
		<Card
			onClick={() => {
				if (!isPending) {
					downloadFile({ originalFilename: it.originalName, fileId: it.id });
				}
			}}
			key={it.id}
			className={cn(
		...
		>
			<div className="relative flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-secondary lg:h-12 lg:w-12">
				{isPending && (
					<Loader className="h-5 w-5 text-foreground animate-spin" />
				)}
